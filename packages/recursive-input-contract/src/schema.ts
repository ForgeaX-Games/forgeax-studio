...
export type RecursiveInputCiProvenance = {
  producerId: string;
...
export type RecursiveInputCiBinding = {
  producerId: string;
  outputContractVersion: string;
  repository: string;
  revision: string;
  ...
  attempt: string;
  job: string;
  trustScope: Extract<TrustScope, 'ordinary-ci' | 'trusted-base-ci'>;
...
type RecursiveInputResultBase = {
  ...
  /** Optional CI binding; absent in the compatible local/legacy contract. */
...
const ciTrustScopeEnum = { type: 'string', enum: ['ordinary-ci', 'trusted-base-ci'] } as const;
    ...
    properties: {
      ...
      ci: {
        ...
        properties: {
          ...
          outputContractVersion: stringSchema,
          repository: stringSchema,
          revision: stringSchema,
          run: stringSchema,
          attempt: stringSchema,
          job: stringSchema,
          trustScope: ciTrustScopeEnum,
          inputProvenance: {
            type: 'object',
            additionalProperties: false,
...
function isCiProvenance(value: unknown): value is RecursiveInputCiProvenance {
  return isRecord(value)
    && hasOnlyKeys(value, ['producerId', 'outputContractVersion', 'repository', 'revision', 'inputDigest'])
    && typeof value.producerId === 'string'
    && value.producerId.length > 0
    && typeof value.outputContractVersion === 'string'
    && value.outputContractVersion.length > 0
    && typeof value.repository === 'string'
    && value.repository.length > 0
    && typeof value.revision === 'string'
    && value.revision.length > 0
...
function isCiBinding(value: unknown): value is RecursiveInputCiBinding {
  return isRecord(value)
    && hasOnlyKeys(value, [
      'producerId',
      'outputContractVersion',
    ...
    && value.job.length > 0
    && (value.trustScope === 'ordinary-ci' || value.trustScope === 'trusted-base-ci')
    && isCiProvenance(value.inputProvenance);
}
...
export function isRecursiveInputResult(value: unknown): value is RecursiveInputResult {
  ...
  if (value.ci !== undefined && !isCiBinding(value.ci)) return false;
