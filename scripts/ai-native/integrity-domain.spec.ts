...
function fixtureRoot(): string {
  ...
  mkdirSync(dirname(crossPackageTarget), { recursive: true });
  writeFileSync(crossPackageTarget, 'export const createForgeaxApp = () => undefined;\n');
  symlinkSync(packagesRoot, join(root, 'packages'));
  return root;
}
...
describe('derived AI-native integrity domain', () => {
  it('includes load-bearing code, live static resources, and workflow-owned roots', () => {
    ...
    expect(manifest.domain_files).toContain('scripts/ai-native/baseline-state.ts');
  ...
  for (const file of ['control-id.ts', 'baseline-state.ts']) {
  ...
  it('rejects a new value import until the generated domain is refreshed', () => {
    const root = fixtureRoot();
    const importer = join(root, 'scripts/ai-native/baseline-state.ts');
