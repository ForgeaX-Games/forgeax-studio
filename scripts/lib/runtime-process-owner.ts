...
export function windowsSnapshotFromProcessTable(pid: number, output: string): RuntimeProcessSnapshot | null {
  let value: unknown;
  try {
  ...
  for (const row of value) {
    // Win32_Process always includes the System Idle Process (PID 0). It is a
    // valid table row but can never be a managed target; do not let it poison
    // validation for every real process in the snapshot.
    if (isWindowsSystemRow(row)) continue;
...
function isWindowsSystemRow(value: unknown): boolean {
  return Boolean(
    value
      && typeof value === 'object'
      && !Array.isArray(value)
      && (value as Record<string, unknown>).ProcessId === 0,
  );
}
