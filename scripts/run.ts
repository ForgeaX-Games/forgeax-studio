...
//   server (:18900) → interface (:18920) + engine (:15173) + editor (:15280)
...
if (process.env.FORGEAX_VITE_NO_CLEAN !== '1') {
  ...
  const editorSharedSrc = join(ROOT, 'packages/editor/packages/editor-shared/src');
  const editorCoreSrc = join(ROOT, 'packages/editor/packages/editor-core/src');
  ...
  viteGuard(ROOT, join(ROOT, 'packages/studio/node_modules/.vite'), 'studio', [engineDist, interfaceSrc, rootLock]);
  viteGuard(ROOT, join(ROOT, 'packages/editor/packages/play-runtime/.vite'), 'play-runtime', [
    engineDist, playSrc, editorSharedSrc, editorCoreSrc, rootLock,
  ]);
  viteGuard(ROOT, join(ROOT, 'packages/editor/packages/edit-runtime/.vite'), 'edit-runtime', [
    engineDist, editSrc, editorSharedSrc, editorCoreSrc, rootLock,
  ]);
}
...
const allocated = new Set<number>([PORT_SERVER, PORT_INTERFACE, PORT_ENGINE, PORT_EDITOR]);
...
// ── 4 launch services ─────────────────────────────────────────────────────────
console.log(
  `[run] starting server :${PORT_SERVER} + interface :${PORT_INTERFACE} + engine :${PORT_ENGINE} + editor :${PORT_EDITOR}`,
);
...
writeFileSync(
  ...
  [
    ...
    `FORGEAX_RUN_PIDS="${[srv, ui, en, ed, narr, ...pluginPids].filter(Boolean).join(' ')}"`,
    `FORGEAX_RUN_PORTS="${[PORT_SERVER, PORT_INTERFACE, PORT_ENGINE, PORT_EDITOR, PORT_NARRATIVE, ...plugins.map((p) => p.frontendPort), ...plugins.map((p) => p.backendPort)].join(' ')}"`,
