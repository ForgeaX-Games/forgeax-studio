...
describe('instance-scoped stop discovery', () => {
  ...
  test('production discovery refuses an unrelated listener and accepts an owned route', () => {
    const target = instance(1);
    const result = scope(target, stateFor(target));
    const port = result.ports[0]!;
    const refused = discoverStopTargets(result, {
    ...
    expect(refused.found.size).toBe(0);
